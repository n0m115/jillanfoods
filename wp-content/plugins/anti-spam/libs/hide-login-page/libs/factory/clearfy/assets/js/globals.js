...
(function($) {
	...
	$.wbcr_factory_clearfy_236.app = $.wbcr_factory_clearfy_236.app || {
		...
		showNotice: function(message, type) {
			...
			$([document.documentElement, document.body]).animate({
				 ...
				 * Пример регистрации хука $.wbcr_factory_clearfy_236.hooks.add('wbcr/factory_clearfy_236/updated',
				 * function(noticeId) {});
				 * @param {string} noticeId - id уведомления
				 */
				$.wbcr_factory_clearfy_236.hooks.run('wbcr/factory_clearfy_236/showed_notice', [noticeId]);
				$.wbcr_factory_clearfy_236.hooks.run('wbcr/clearfy/showed_notice', [noticeId]);
			});
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wbcr_factory_clearfy_236.hooks.run('wbcr/factory_clearfy_236/hidded_notice', [noticeId]);
				$.wbcr_factory_clearfy_236.hooks.run('wbcr/clearfy/hidded_notice', [noticeId]);
			});
		},
	...
	$.wbcr_factory_clearfy_236.filters = $.wbcr_factory_clearfy_236.filters || {
	...
	$.wbcr_factory_clearfy_236.hooks = $.wbcr_factory_clearfy_236.hooks || {

		/**
		 * Applies filters to a given input value.
		 */
		run: function(filterName, args) {
			$.wbcr_factory_clearfy_236.filters.run(filterName, args);
		},
		 ...
		 */
		add: function(filterName, callback, priority) {
			$.wbcr_factory_clearfy_236.filters.add(filterName, callback, priority);
		}
	};
